 ...
 * 本物のChromeで記事259本を測った結果(フォント読み込み後):
 *   画面幅1280px  1行 188本(73%) / 2行 71本 / 3行以上 0本
 *   画面幅 834px  1行 194本(75%) / 2行 65本 / 3行以上 0本
 * 何もしていなかったときは1280pxで1行が67本(26%)だったので、およそ3倍になった。
 * 下限を21pxにしたのは、本文が16pxで、これ以上下げると題が本文と見分けにくく
 * なるため。残る71本は題そのものが長いので、収めるには題を短くするしかない
 * (公開済みの題を変えるのは本人の許可が要る)。
 *
 * 測るときの注意。**Webフォントの読み込みを待ってから測ること。**
 * DOMContentLoadedの時点では代替フォントで組まれていて字幅が違う。
 * 2026-09-12、待たずに測って「21pxで218本(84%)」という数字を出してしまい、
 * 実機(フォント読み込み後)では188本だった。1行あたり30本ぶんずれる。
 *
