...
async def check_page(context, url_path, is_mobile, sem, findings):
    async with sem:
        ...
        try:
            ...
            broken = c["brokenImgs"]
            # 404.htmlの画像は /ozu-life-memo/... の絶対パスで書いてある(理由は下のコメント)。
            # ローカルの点検サーバーでだけ読めないので、この1ページに限って除外する。
            if url_path == "404.html":
                broken = [b for b in broken if "/ozu-life-memo/" not in str(b)]
            if broken:
                issues.append(f"壊れた画像: {broken[:3]}")
            if c["hOverflow"]:
