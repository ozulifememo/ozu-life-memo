...
def strip_blockquotes(html_text: str) -> str:
    """<blockquote> の中身を落とす。

    引用した答弁は敬体で、長く、「今年」と言う。それは原文がそうだからで、
    書き手の文体の問題ではない。目安の検査(敬体・1文の長さ・相対的な時期)は
    引用を数えない。2026-09-06に足した。
    """
    return re.sub(r"<blockquote\b.*?</blockquote>", " ", html_text, flags=re.S | re.I)
...
def check_style(path, html, rep, show_headings=False):
    ...
    if kind == "article":
        text = strip_tags(strip_blockquotes(body))
...
def check_phrasing(path, html, rep):
    ...
    # 長すぎる1文
    worst, worst_len = "", 0
    for m in re.finditer(r"<p(?:\s[^>]*)?>(.*?)</p>", strip_blockquotes(body),
                         flags=re.S | re.I):
...
def check_raw_quote(path, html, rep):
    ...
    2026-09-06。原稿は Markdown の「> 」で引用を書くのに、kiji_md.py が
    それを知らず、&gt; のまま <p class="commentary"> に落としていた。
    公開ページに「> 」という記号がそのまま見えていた記事が8本あった。
    正解は1つ(引用は blockquote class="article-quote" になる)なのでエラー。
    """
    if page_type(path) not in ("article", "book", "kenkyu"):
        return
    for m in re.finditer(r'<p[^>]*>\s*&gt;\s*(.{0,60})', body_only(html)):
        rep.error(rel(path), "引用",
                  "引用が変換されず「&gt;」のまま本文に出ています",
                  "       …" + " ".join(m.group(1).split()) + "…\n"
                  "       kiji_md.py で作り直すか、手で blockquote にする")
        break
    # 表も同じ形の事故を起こす。2026-09-06、Markdown の表がそのまま
    # 段落に落ちて、罫線の記号がページに並んでいた記事が2本あった
    for m in re.finditer(r'<p[^>]*>\s*\|(.{0,60})', body_only(html)):
        rep.error(rel(path), "引用",
                  "表が変換されず「|」のまま本文に出ています",
                  "       …|" + " ".join(m.group(1).split()) + "…\n"
...
def check_relative_dates(path, html, registry, rep):
    ...
    if not reg.get("date") or reg["date"] < RELATIVE_WORDS_SINCE:
        return
    jibun = strip_quotes(strip_tags(strip_blockquotes(body_only(html))))
...
REQ_SAYS = ("呼びかけ", "と述べ", "と答弁", "という声", "と話し", "としている",
            ...
            # 議会で議員が質問した内容の引用。書き手の要望ではない
            # (2026-09-07、ozu-saiban が誤検知された)
            "と問われ", "と質問", "と質し", "と迫", "と指摘され", "との質疑")
...
#   同じ日に「もうちょいゆるくしよか。2800文字にしよか」と言われ、
#   しきい値を3,000字から2,800字に下げた。
#
...
TANBUN = 2800     # 2026-09-06、本人が3,000字から少しゆるめた
...
SHUZAI_OTHER = ("が取材", "の取材", "が確認", "が問い合わせ", "による取材",
                "が聞い", "の聞き取り", "が実施した")
# 公開されているデータベースや資料を、自分で開いて調べた形。
# これは取材ではない(2026-09-07、ozu-nandoku-chimei が誤検知された)。
# 例:「国税庁の法人番号公表サイトで大洲市に本店を置く法人1,607件の登記住所を調べたところ」
SHUZAI_OPENDATA = ("公表サイト", "データベース", "データセット", "検索サイト",
                   "会議録", "例規集", "統計", "オープンデータ", "e-Stat",
                   "登記", "公報", "官報", "を調べたところ", "で調べたところ")
...
def check_shuzai(path, html, rep):
    ...
    for sent in re.split(r"(?<=。)", jibun):
        ...
        if any(x in sent for x in SHUZAI_OTHER):
            continue
        if any(x in sent for x in SHUZAI_OPENDATA):
            continue
        for d in SHUZAI_DO:
            if d in sent:
...
LOCAL_PATH = [
    (r"[A-Za-z]:[\\/]Users[\\/]", "Windowsのユーザーフォルダの絶対パス"),
    (r"/(?:home|Users)/[A-Za-z0-9._-]{2,}/", "ホームディレクトリの絶対パス"),
    (r"Desktop[\\/][^\s\"\'<>]{2,}", "デスクトップのフォルダ名"),
    (r"AppData[\\/]Local[\\/]Temp", "作業用の一時フォルダ"),
]
...
def check_local_paths(rep):
    ...
    2026-09-06。tools/_cssscope.py にユーザー名つきの絶対パスが
    9月3日から載っていた。tools/status-state.json にもデスクトップの
    フォルダ名が入っていた。どちらも匿名性の手がかりになる。
    """
    import subprocess
    ...
    try:
        ...
        out = ""
    if not out.strip():
        # git が無いところ(自己診断の仮フォルダなど)では、そのまま歩いて見る。
        # 「見ているつもりで見ていない」検査を作らないため
        names = []
        for q in REPO.rglob("*"):
            if q.is_file() and not any(x.startswith(".") for x in q.parts):
                names.append(str(q.relative_to(REPO)).replace(chr(92), "/"))
        out = "\n".join(names)
    for f in out.split("\n"):
        if not f or f.lower().endswith(skip_ext):
            continue
        if f.endswith("tools/check_site.py"):
            # この検査器自身は、わざと壊した検体(BROKEN_SAMPLE)を抱えている。
            # 自分の検体で自分が鳴るので外す。check_repo_anonymity が
            # ALLOW で同じことをしているのと同じ理由
            continue
        p = REPO / f
        if not p.exists():
            continue
        try:
            ...
            continue
        for pattern, why in LOCAL_PATH:
            m = re.search(pattern, t)
            if not m:
                continue
            around = " ".join(t[max(0, m.start() - 30): m.end() + 40].split())
            rep.error(f, "匿名性",
                      "開発機のローカルパスが入っています(" + why + ")",
                      "       …" + around[:110] + "…\n"
                      "       どの端末のどのユーザーかが分かる。相対パスか引数にする")
            break
